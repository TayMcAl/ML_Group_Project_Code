...
class Mlp():
    ...
    def train(self, X, Y, iterations=50, reset=False):
        '''
...
X1, Y1, labels1, y1  = load_test()

tic = time.time()

# Creating the MLP object initialize the weights
mlp_classifier = Mlp(size_layers = [784, 20, 10], 
                         act_funct   = 'sigmoid',
                         reg_lambda  = 0,
                         bias_flag   = True)
...
for ix in range(epochs):
    ...
    Y_hat1 = mlp_classifier.predict(X1)
    # loss
    ...
    train_acc[ix] = acc
    print('Training Accuracy on Train: ' + str(acc*100))
    
    # Training Accuracy
    #Y_hat1 = mlp_classifier.predict(X)
    y_tmp1 = np.argmax(Y_hat1, axis=1)
    y_hat1 = labels1[y_tmp1]

    acc = np.mean(1 * (y_hat1 == y1))
    test_acc[ix] = acc
    print('Training Accuracy on Test: ' + str(acc*100))
...
plt.figure()
ix = np.arange(epochs)
plt.plot(ix, train_acc)
plt.plot(ix, test_acc)
